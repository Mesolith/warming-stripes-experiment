...
In the original version for Oxford they then fillet to exclude
              ...
              mutate(anomaly=tave-ref_ave) %>%
                ggplot(aes(x=Year,fill=anomaly,y=1))+
                   geom_col(show.legend = FALSE,width=1)+
                     scale_fill_fermenter(palette="RdBu",limits=c(-1.5,1.5))+
                        theme_void()+
                           labs(fill="Temperature Anomaly",title="Nair Warming Stripes",subtitle = "1931-2021")
```
...
This is still biased by the rainfall minimum in the 1970's where we start the reference value, so for refmeanann take an average of all of th annual rainfall data.
